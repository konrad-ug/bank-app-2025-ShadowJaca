...
@step('Account with pesel "{pesel}" has balance equal to "{balance:f}"')
def balance_equals_to(context, pesel, balance):
    response = requests.get(URL + f"/api/accounts/{pesel}")
    assert response.status_code == 200
    assert response.json()["balance"] == balance

@step('I do "{ttype}" transfer of amount "{amount:f}" to account with pesel "{pesel}"')
def do_transfer(context, ttype, amount, pesel):
    json_body = {
        "amount": amount,
        "type": ttype
    }
